...
\begin{enumerate}[itemsep=2pt]
  \item Evidence that both anomalies are linearly decodable states, and
        that word-list directions on the same activations miss them.
  \item A causal induction result: the learned direction turns the body
        behaviour on in fluent text, and a sham direction does not.
  ...
  \item An instrument finding: a forced-choice letter probe confabulates
        from conversation content, while free-text answers from the same
        state honestly say nothing was injected.
\end{enumerate}
