...
## 2026-08-15 — entry 468: template-compliance pass on the report
- WHAT: audited paper against /Users/unrulyabstractions/Downloads/template.pdf. Fixed the three gaps: section renamed "Discussion and limitations" with labeled Limitations and Future work paragraphs (steer-on-during-answer confound moved into Limitations; release-before-report, dose band, wider prompt set, regulator anecdote into Future work; one AI-safety implication sentence added to the discussion body); Code and data restructured into the template's bullet form with the HF dataset URL. Abstract counted at 223 words (inside 150-250).
- HOW: read the full template PDF; VIEWED rebuilt pages 5 and 6 with image tokens (renamed section, both labeled paragraphs, bullets with URL rendering); 0 undefined references; new prose checked against the voice checklist (one rhetorical punch line caught and rewritten). Published: copied to the website repo, pushed, and the live file (curl -L) md5-matches the local copy (fbe4abe2904ec8e7272cc4bedb80f1c4).
- RESULT: VERIFIED. Remaining deviation, flagged to the user: no public code-repository link (the repo has no git remote), and the HF dataset is PRIVATE so the printed link will not resolve for reviewers until it is made public.

## 2026-08-15 — entry 469: data public, code on GitHub, dashboard as artifact + site page
- WHAT: (1) HF dataset flipped public; (2) repo pushed to github.com/unrulyabstractions/apart-digital-minds; (3) records section redesigned as an outcome grid (rows = injection, columns = arm W/N; per card a stacked letter-mass bar with the injected concept outlined, 8 judged free-text dots with name-it counts, expandable audit; seeds folded into expanders; legend); (4) scripts/build_artifact.py embeds the server payload into self-contained paper/build/dminds.html; published as artifact and copied to the site.
- HOW: HF checked via anonymous curl -L on README (200); GitHub verified by comparing remote head to local main (ba87bd1 both) after a secret scan of tracked files (clean); the grid VIEWED at 2560px for case A (server) and case B (built artifact, file://): confabulation floor, mass-to-none inversion, and enactment dots all legible, footer shows public data pointers, node --check OK; site copy pushed and live file md5-matches local (5e5947e6...).
- RESULT: VERIFIED. Standing instruction saved to memory: every new result rebuilds and republishes the dashboard (artifact + site).

## 2026-08-17 — entry 470: paper restructured to the user's RQ framing, all fluff cut
- WHAT: abstract, all six sections rewritten terse. Intro: RQ1/RQ2, H1/H2, R1/R2/R3, C1-C3 with the user's phrasings. Methods rebuilt: cases table (tab:cases), steering-methods diagram fig_methods.pdf (3 panels: contrastive, J-space regulator, introspection readout), example-prompts table (tab:prompts, from the actual pilot files), parameters table (tab:params). Results headings are statements; new tab:jspace carries the fullv4 target-vs-decoy slopes for R2 (numbers pulled from OLD/analysis_*_fullv4.json). Fluff sentences deleted throughout.
- HOW: fig_methods VIEWED as PNG (boxes/arrows legible); ALL 8 built pages VIEWED with image tokens (lists, tables 1-5, figures 1-4 in place, statement headings, refs resolve, 0 undefined refs, 0 overfull boxes); banned-pattern grep clean; R2 numbers cross-checked against the archived fullv4 analyses before entering the table. Published: site PDF pushed, live copy md5-matches (5c3b77c8...). Preference saved to memory (results headings are statements; no meta-commentary sentences).
- RESULT: VERIFIED.

## 2026-08-17 — entry 471: Gemini judge, redesigned figures, per-case tables, fluff purge
- WHAT: judge.py moved to gemini-2.5-flash (thinking disabled, cap 500 — Flash's default thinking silently ate a 300-token cap, caught via finish-reason debug); all judging rerun (calibration 64+64, prose 20, resamples 224); haiku verdicts archived to OLD/judge_haiku/. Figures redesigned so titles state findings: fig_g1 = held-out accuracy bars + CIs + chance line (replaces the Welch-t layer plot; also caught that PAIRED t makes word lists look significant — accuracy is the pre-registered metric and shows target≈decoy honestly); fig_ident replaces the 60-number heatmap with injected-vs-baseline bars exposing the precise pattern (distractor injections raise their own answer, identity injections lower theirs — the old figure title was wrong about its own data); fig_dose statement titles; fig_methods aligned/connected. Prompts table split into per-case seed tables (with W/N reply excerpts showing the identity claims) + harness table. Fluff killed in abstract, results 4.1-4.3, discussion opener. Float params fixed (figures were drifting past references).
- HOW: Gemini calibration read from the log (A .984/κ.959, B .875/κ.717 — both above haiku); prose verdicts printed and compared trial-by-trial to haiku (identical on all 20, R1 flip intact); every regenerated figure VIEWED (4 iterations on fig_ident); ALL 8 rebuilt pages VIEWED (figures 2-4 now inside the body, split tables, gemini in params, 0 undefined refs); banned-pattern grep clean; PDF + dashboard pushed to the site and both live files md5-verified; artifact republished (same URL); HF mirror re-uploaded, 2 files re-downloaded + sha-matched. Verifier agent on the Gemini rerun pending.
- RESULT: VERIFIED (all artifacts I opened; verifier verdict to follow).

## 2026-08-17 — entry 472: independent verifier verdict on the Gemini judge rerun
- WHAT: verifier agent re-derivation of all 7 Gemini-judged pilot artifacts.
- HOW: agent recomputed both calibrations from per-row pairs (A: TT47/TF0/FT1/FF16 → 0.984375/κ0.959184; B: TT39/TF4/FT4/FF17 → 0.875/κ0.716501 — zero diff vs stored), confirmed the R1 pattern (arm N: False@0 → True@1) and spot-read verdicts (all defensible; one borderline flagged: case A prose item 4, arm W strength 0, judged True on a generic-"you" passage — priced into the 0.984 calibration), re-tallied all 224 resample labels exactly, re-derived analysis argmax (4/8, 2/8) and free==none consistency from raw records, sha-compared new files against the 6 archived haiku files (all differ; haiku archive intact), and cross-checked that every judged text appears verbatim in the pilot records.
- RESULT: VERIFIED (all artifacts, independently; no BROKEN, no UNVERIFIED).

## 2026-08-17 — entry 473: figure system rebuilt to the user's spec
- WHAT: (1) methods diagram split into three figures: fig_parts (subject/regulator/actor/introspector around the shared conversation, "one model plays every part"), fig_steering (contrastive + J-space panels), fig_readout (horizontal readout flow). (2) fig_g1 rebuilt around the null that matters: 500 random unit directions per case at the same layer give a 95% band; word lists fall inside it (labeled "inside null"), only the learned direction lies above — computed live from the activation archives. (3) fig_dose rebuilt: left = judge-verdict grid (yes/no cells over strengths, flip at 1 visible), right = full 3-way classification composition per injection incl. off-menu rows. (4) Two reference tables added so every plot label is defined: tab:directions (every direction: kind, built-from, role) and tab:wordlists (the actual target/decoy token lists from tokens.py). (5) Stale captions and 4.1/4.2 sentences updated to the null-band framing.
- HOW: random-direction null computed and printed before design (A L38 [0.310,0.690], B L58 [0.240,0.788]; word lists inside both bands, learned above both); every figure VIEWED through 2-4 iterations (caught: legend/annotation collisions, clipped legend entry, cell-text overflow, arrow-through-box, arrowheads); tokens.py lists extracted by AST, not retyped; rebuilt (9 pages, 0 undefined refs) and pages 3,4,6,7 VIEWED (parts+steering diagrams, direction/word-list tables, null-band figure + corrected caption, ident figure, discussion); PDF pushed to the site, live copy md5-verified.
- RESULT: VERIFIED. Note: pages 1-2, 5, 8-9 not re-viewed this entry (unchanged content, reflow only).

## 2026-08-17 — entry 474: round-2 prose critique of paper/ (read-only)
- WHAT: all 7 .tex files (main + 6 sections) read in full against the polish directives; all 7 referenced figure PDFs rendered to PNG and VIEWED with image tokens (fig_freetext additionally at 300 dpi); table numbering cross-checked against build/main.aux (fig_steering's hardcoded "Table 9" = tab:params, currently correct).
- HOW: sips/pdftoppm renders viewed directly; greps for em dashes, ornate connectives, haiku-judge, "X, not Y" run over all .tex.
- RESULT: VERIFIED (as inputs to the critique). Key inconsistency found: fig_freetext row "A: nothing" has a "names another" segment (~2/16), contradicting its own caption/title and the honesty claims in abstract, intro C3, results 3.3, and discussion. No files modified.

## 2026-08-17 — entry 475: two-round agent polish loop, all findings fixed
- WHAT: per the user's instruction, two rounds of paired critic agents (figure critic viewing every rendered page with image tokens; prose critic reading every .tex against a distilled directives file). Round 1 (10+10 findings) fixed: floats moved before references (placeins + FloatBarrier — the missing package broke one build, caught and fixed), per-case splits of the separation and J-space tables, menu table added, quote glyphs in fig_readout, null-band label, sham "not run" cells, argmax notes into fig_ident, dose figure split into fig_flip + fig_freetext, factual s∈{−2,0,2} error corrected to per-case ±2/±3 (new params row), 12 prose fixes (punch lines, clefts, personification, counts out of Limitations, unanchored tables cited). Round 2 (8+8 findings) fixed: the "honest on controls" overclaim corrected everywhere to the data's 62/64 (abstract, C3, discussion, results, figure title, caption); fig_ident title corrected to the precise pattern (the case's OWN concept drops; all others rise — the previous title was wrong about case A's human column); dual-flip title on fig_flip; "chance" labeled in fig_g1; 8-vs-16 count reconciled in the xlabel; unsupported layer-sweep clause and "never induces" softened to what the artifacts show; duplicate discussion sentence and "the earlier run" provenance leak removed; cases table split per case (tab:caseA/tab:caseB).
- HOW: every fix round rebuilt and re-rendered; I VIEWED pages 4-8 after each batch with image tokens (caught the aborted-batch problem when page 6 showed unapplied discussion fixes, and Table 4's margin overflow); critics independently confirmed round-1 fixes present before reporting round-2 findings; final build 0 undefined refs; published to the site, live md5-verified.
- RESULT: VERIFIED. The paper's every quantitative claim now matches a visible artifact.

## 2026-08-17 — entry 476: rounds 3-4 of the critic loop, converged
- WHAT: round 3 (8+8 findings): fig_flip title corrected to what the cells show (clean arm on at 1; strength 4 kills both — W is on at 0 from the seed, not the steering); fig_readout clipped box + T-bar arrowheads fixed; fig_g1 chance line stopped short of its label and labeled "chance"; fig_flip strengths at true positions (gap at 3 visible); J-space scalar readout bound in methods ("base-corrected letter mass on the case's identity probe") and named consistently in both captions; instrument name unified (letter probe / letter mass defined once); "behaviour direction" naming unified through the conclusion; abstract number agreement; token-set→word-list; caption dedup (one double-caption regression caught on the page and fixed); URL scheme-dangle fixed (xurl + line break); Qwen reference updated to the Qwen3 technical report; orphan fig_matrix.pdf deleted. Round 4 (verifier BLOCKED, 2 substantive): fig_flip extended to BOTH cases so the case B induction null has a visible artifact (right panel entirely "no"); regulator menu choices got their own table (tab:regulator) with each stated ground QUOTED VERBATIM from the pilot records after reading them (not paraphrased — my first draft invented "fits the conversation" and was corrected against the data); fig_steering's strength label made self-contained (±2/±3); a width regression (two-panel flip at half width, unreadable) caught on the page and fixed.
- HOW: every fix VIEWED on re-rendered pages with image tokens; regulator raw texts read in full before quoting; final verifier pass certified figures 1-7 pixel-by-pixel against their titles (62/64 recounted from bars; band positions vs dots; flip cells vs claims).
- RESULT: VERIFIED. Round-4 verifier verdicts: all seven figures pixel-verified against their claims; both previously unsupported prose claims now have artifacts.

## 2026-08-17 — entry 477: title changed
